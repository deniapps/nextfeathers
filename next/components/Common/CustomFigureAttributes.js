...
export default class CustomFigureAttributes {
  ...
  afterInit() {
    ...
    // editor.conversion
    //   .for("upcast")
    //   .add(upcastCustomClasses("figure"), { priority: "low" });
...
function setupCustomClassConversion(viewElementName, modelElementName, editor) {
  ...
  // editor.conversion
  //   .for("downcast")
  //   .add(downcastCustomClassesToFigure(modelElementName), { priority: "low" });
  editor.conversion
    .for("downcast")
    .add(downcastCustomClassesToChild(viewElementName, modelElementName), {
      priority: "low",
    });
}
...
function upcastCustomClasses(elementName) {
  return (dispatcher) =>
    dispatcher.on(`element:${elementName}`, (evt, data, conversionApi) => {
      ...
      const currentAttributeValue = modelElement.getAttribute(
        "customClass"
      ) || ["ui", "table", "celled"];
