(function () {
  ...
  let SandBox = (window.SandBox = class {
    ...
    constructor(frame) {
      ...
      this.loaded = window.loadedModules = new Cached((name) =>
        resolved
          .compute(name)
          .then(({ name, code }) => this.evalModule(name, code))
      );
    ...
    run(code, output, meta) {
      ...
      this.extraSecs =
        typeof code == "string" && /promtDirection/.test(code) ? 0.1 : 2;
      this.win.__c = 0;
      this.prepare(code)
        .then((code) => {
    ...
    prepare(text) {
      ...
      return (this.handleDeps
        ? Promise.all(dependencies.map((dep) => this.loaded.compute(dep)))
        : Promise.resolve([])
      ).then(() => code);
    }
    ...
    evalModule(name, code) {
      ...
      let work = findDeps(code).map((dep) =>
        this.loaded.compute(resolveRelative(name, dep))
      );
      return Promise.all(work).then(() => {
        let f = new this.win.Function(
          "require, exports, module, __dirname, __filename",
          code + "\n//# sourceURL=code" + randomID()
        );
        let module = this.loaded.store(name, { exports: {} });
        f(
          (dep) => this.require(resolveRelative(name, dep)),
          module.exports,
          module,
          name,
          name
    ...
    setHTML(code, output, callback) {
      ...
      doc.documentElement.innerHTML = code.replace(
        /<script\b[^>]*?(?:\bsrc\s*=\s*('[^']+'|"[^"]+"|[^\s>]+)[^>]*)?>([\s\S]*?)<\/script>/g,
        function (_, src, content) {
      ...
      function loadScript(i) {
        ...
        if (tag.src) {
          ...
          let id = randomID();
          sandbox.callbacks[id] = () => {
            delete sandbox.callbacks[id];
            loadScript(i + 1);
          };
    ...
    setupEnv() {
      ...
      win.requestAnimationFrame = (f) => {
        ...
        if (this.frames.length > 50) this.frames[this.framePos++ % 50] = val;
        else this.frames.push(val);
        return val;
      };
      ...
      win.require.preload = (name, code) =>
        resolved.store(name, { name, code });
      win.module = { exports: {} };
    ...
    resizeFrame() {
      this.frame.style.height =
        Math.max(
          80,
          Math.min(this.win.document.documentElement.offsetHeight + 10, 500)
        ) + "px";
    ...
    tick() {
      ...
      let bail = confirm(
        "This code has been running for " +
          this.extraSecs +
          " seconds. Abort it?"
      );
    ...
    error(exception) {
      ...
      if (stack.length > 1) {
        ...
        let mark = this.output.div.lastChild.appendChild(
    ...
    clearEvents() {
      ...
      while (this.intervals.length)
        this.win.clearInterval(this.intervals.pop());
      while (this.frames.length)
        this.win.cancelAnimationFrame(this.frames.pop());
  ...
  function preprocess(code, sandbox) {
    ...
    if (!/\n$/.test(code)) code += "\n";
    let strict = /^(\s|\/\/.*)*["']use strict['"]/.test(code),
      ast;
    try {
      ast = acorn.parse(code, { sourceType: detectSourceType(code) });
    } catch (e) {
      return { code, dependencies: [] };
    }
    let patches = [];
    ...
    acorn.walk.simple(ast, {
      ...
      CallExpression(node) {
        if (
          ...
          !dependencies.includes(node.arguments[0].value)
        )
          dependencies.push(node.arguments[0].value);
      },
      ImportDeclaration(node) {
        dependencies.push(node.source.value);
        let req = "require(" + node.source.raw + ")",
          text;
        if (node.specifiers.length == 0) {
          ...
          node.specifiers.forEach((spec) => {
            if (spec.type == "ImportDefaultSpecifier")
              text +=
                ", " + spec.local.name + " = " + name + ".default || " + name;
            else if (name != null)
              text +=
                ", " +
                spec.local.name +
      ...
      ExportDefaultDeclaration(node) {
        if (/Declaration/.test(node.declaration.type)) {
          ...
          patches.push(
            { from: node.start, to: node.declaration.start, text: ";(" },
      ...
      if (stat.type == "VariableDeclaration" && stat.kind != "var") {
        if (stat.kind == "const") {
          ...
          if (found)
            patches.push({
              from: 0,
              text: `throw new TypeError("invalid assignment to const '${found}'");`,
            });
        }
        patches.push({
          from: stat.start,
          to: stat.start + stat.kind.length,
          text: "var",
        });
      }
    ...
    patches.push({
      from: catchPos,
      text: "}catch(e){__sandbox.error(e);}",
      priority: -10,
    });
    patches.sort(
      (a, b) =>
      ...
      acorn.walk.simple(
        decls[i].id,
        {
          VariablePattern(node) {
            if (findAssignments(node.name, ast)) found = node.name;
          },
        },
        null,
        null,
    ...
    acorn.walk.ancestor(ast, {
      AssignmentExpression(node, ancestors) {
        ...
        for (let i = 2; i < ancestors.length; i++)
          if (/Statement|Declaration/.test(ancestors[i].type)) return;
        found = true;
      },
    });
    return found;
    ...
    acorn.walk.simple(ast, {
      CallExpression(node) {
        if (
          ...
          node.arguments[0].type == "Literal" &&
          typeof node.arguments[0].value == "string" &&
          !deps.includes(node.arguments[0].value)
        )
  ...
  class Cached {
    ...
    compute(value) {
      return (
        this.work[value] ||
        (this.work[value] = this.mapping(value).then(
          (result) => (this.done[value] = result)
        ))
      );
    }
  ...
  const resolved = new Cached((name) =>
    fetch("https://unpkg.com/" + name.replace(/\/$/, "")).then((resp) => {
      if (resp.status >= 400)
      ...
      return (
        known ||
        resp.text().then((code) => resolved.store(found, { name: found, code }))
      );
    })
  );
  ...
  SandBox.Output = class {
    ...
    out(type, args) {
      ...
      for (let i = 0; i < args.length; ++i) {
        ...
        else wrap.appendChild(represent(arg, 58));
      }
  ...
  function representArray(val, space) {
    ...
    for (let i = 0; i < val.length; ++i) {
      ...
      if (space - nextSize <= 0) {
        wrap
          .appendChild(span("etc", "…"))
          .addEventListener("click", () => expandObj(wrap, "array", val));
        break;
      }
      space -= nextSize;
      ...
      for (let prop in val)
        if (hop(val, prop)) {
          ...
          if (space - nextSize <= 0) {
            wrap
              .appendChild(span("etc", "…"))
              .addEventListener("click", () => expandObj(wrap, "obj", val));
            break;
          }
          space -= nextSize;
  ...
  function expandObj(node, type, val) {
    ...
    function addProp(name) {
      ...
      row
        ...
        .appendChild(span("prop", name + ":"));
      row
        .appendChild(document.createElement("td"))
        .appendChild(represent(val[name], 40));
    }
    if (type == "array") {
