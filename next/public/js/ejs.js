window.addEventListener("load", () => {
  ...
  if (
    ...
    window.chapNum < 20 &&
    window.localStorage &&
    !localStorage.getItem("usedSandbox")
  ) {
    let pres = document.getElementsByTagName("pre");
    for (let i = 0; i < pres.length; i++) {
      let pre = pres[i];
      if (
        !/^(text\/)?(javascript|html)$/.test(
  ...
  CodeMirror.commands[(CodeMirror.keyMap.default.Down = "lineDownEscape")] = (
    cm
  ) => {
  ...
  CodeMirror.commands[(CodeMirror.keyMap.default.Up = "lineUpEscape")] = (
    cm
  ) => {
    let cur = cm.getCursor();
  ...
  let keyMap = {
    ...
    "Ctrl-Enter"(cm) {
      runCode(cm.state.context);
    },
    "Cmd-Enter"(cm) {
      runCode(cm.state.context);
    },
    "Ctrl-Down"(cm) {
      closeCode(cm.state.context);
    },
    "Ctrl-Esc"(cm) {
      resetSandbox(cm.state.context.sandbox);
    },
    "Cmd-Esc"(cm) {
      resetSandbox(cm.state.context.sandbox);
    },
  ...
  function activateCode(node, e, lang) {
    ...
    let code =
      (window.localStorage && localStorage.getItem(codeId)) || node.textContent;
    let wrap = node.parentNode.insertBefore(
      elt("div", { class: "editor-wrap" }),
      node
    );
    ...
    if (window.localStorage)
      editor.on(
        "change",
        debounce(() => localStorage.setItem(codeId, editor.getValue()), 250)
      );
    wrap.style.marginLeft = wrap.style.marginRight =
      -Math.min(article.offsetLeft, 100) + "px";
    setTimeout(() => editor.refresh(), 600);
    if (e) {
      editor.setCursor(
        editor.coordsChar({ left: e.clientX, top: e.clientY }, "client")
      );
      editor.focus();
    }
    let out = wrap.appendChild(elt("div", { class: "sandbox-output" }));
    let menu = wrap.appendChild(
      elt("div", { class: "sandbox-menu", title: "Sandbox menu..." })
    );
    ...
    function click(e) {
      ...
      if (e.target.parentNode == menu) {
        for (let i = 0; i < menu.childNodes.length; ++i)
          if (target == menu.childNodes[i]) items[i][1]();
      }
  ...
  function runCode(data) {
    ...
    getSandbox(data.sandbox, data.isHTML, (box) => {
      ...
      else box.run(val, data.output, data.meta);
    });
  }
  ...
  let sandboxSnippets = {};
  {
    ...
    for (let i = 0; i < snippets.length; i++) {
      ...
      if (
        snippet.getAttribute("data-language") == "text/html" &&
