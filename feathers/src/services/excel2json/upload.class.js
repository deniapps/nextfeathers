...
class Service {
  ...
  async create (data, params) {
    ...
    const companyData = newData.reduce((arr, member) => {
      if(typeof (companies.find(com => com.value === member[1])) === 'undefined'){
        console.log('yes');
      ...
      arr[member[1]] ? arr[member[1]].push(newMember) : arr[member[1]] = [newMember];
      return arr;
    }, {});
    ...
    fs.writeFile('companyLookups.json', JSON.stringify(companyLookups), function(err) {
      if (err) {
        console.log(err);
      }
    });
